...
/**
 * Provides abstraction for <code>local_variable_table_info</code> item
 * 
 * <p>The <code>local_variable_table_info</code> is a fixed-length sub-element structure used in <code>LocalVariableTable_attribute</code> item. 
 * Each entry in the local_variable_table array indicates a range of code array offsets within which a local variable has a value. It also 
 * indicates the index into the local variable array of the current frame at which that local variable can be found.</p>
 * <p>The <code>local_variable_table_info</code> structure has the following format:</p>
 * <code>local_variable_table_info{<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;u2 start_pc;<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;u2 length;<br>
 * &nbsp;&nbsp;&nbsp;&nbsp;u2 name_index;<br>
...
{
	/**
	 * <p>The value of <code>start_pc</code> must be a valid index into the <code>code</code> array of this <code>Code_attribute</code> and must 
	 * be the index of the opcode of an instruction.</p>
	 */
	...
	/**
	 * <p>The value of the <code>descriptor_index</code> item must be a valid index into the <code>constant_pool</code> table. The <code>constant_pool</code> 
	 * entry at that index must contain a <code>CONSTANT_Utf8_info</code> structure representing a field descriptor which encodes the type of a 
	 ...
	 * <p>If the local variable at index is of type double or long, it occupies both <code>index</code> and <code>index + 1</code>.
	 */
	private short index;
	/**
	 * Factory method to generate a <code>local_variable_table_info</code> instance.
	 * 
	 * <p>The <code>local_variable_table_info</code> is a fixed-length sub-element structure used in <code>LocalVariableTable_attribute</code> item. 
	 * Each entry in the local_variable_table array indicates a range of code array offsets within which a local variable has a value. It also 
	 * indicates the index into the local variable array of the current frame at which that local variable can be found.</p>
	 ...
	 * @return instance of <code>local_variable_table_info</code>
	 */
	 ...
	 * 
	 * <p>Return size of <code>local_variable_table_info</code> entry components</p>
	 * @return size of <code>local_variable_table_info</code> entry
	 */
	 ...
	 * 
	 * <p>The given local variable must be at index in the local variable array of the current frame.</p>
	 * <p>If the local variable at index is of type double or long, it occupies both <code>index</code> and <code>index + 1</code>.</p>
